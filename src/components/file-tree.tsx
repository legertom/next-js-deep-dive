...
function TreeItem({ item, depth = 0 }: { item: FileTreeItem; depth?: number }) {
  ...
  return (
    <>
      <div
        className={`flex items-center gap-2 py-1 px-3 text-sm font-mono ${item.highlight ? "bg-accent-light/60 text-blue-800 font-semibold rounded" : "text-stone-600"}`}
...
export function FileTree({ items, title }: FileTreeProps) {
  return (
    <div className="my-6 rounded-xl border border-card-border bg-card overflow-hidden">
      {title && (
        <div className="px-4 py-2.5 bg-stone-50 border-b border-card-border text-xs font-semibold text-muted uppercase tracking-wider">
          {title}
        </div>
      )}
      <div className="py-2">
